...
cupid <- read.csv("cupid.csv", header = TRUE, na.strings=c(""," ","NA")) #load data
...
sex_dist <- ggplot(cupid, aes(x = sex, fill= sex)) + geom_bar(stat = "count") + ylab("Number of people") + xlab("Sex") + theme_minimal() + scale_y_continuous(breaks = seq(0, 35000, by = 5000)) + scale_fill_discrete("Sex")
plot(sex_dist) #more man than women
...
status <- ggplot(cupid.status, aes(x = V1, y = value, fill = factor(V2))) +
  geom_bar(stat = "identity", position = "dodge") +
